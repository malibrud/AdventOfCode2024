...
void walkBack( Maze *mz, Node *n ) {
    // if ( mz->grid[ n->y ][ n->x ] == 'O' ) return;
    mz->grid[ n->y ][ n->x ] = 'O';
    for ( int i = 0 ; i < n->prevCount ; i++ ) {
        Node *p = n->prev[ i ];
        if ( p->dir != n->dir && p->score + 1000 == n->score ) 
            walkBack( mz, p );
        else if ( p->score + 1 == n->score ) 
            walkBack( mz, p );
    }
}
...
int main( int argc, char **argv ) {
    ...
    {
        int idx = y*(D*X) + x*D + d;
        nodes[ idx ] = (Node){ UINT32_MAX, x, y, d, 0 };
    ...
    while ( !qIsEmpty( &q ) ) {
        ...
        // Are we done?
        if ( n->x == mz.ex && n->y == mz.ey ) {
            minFinal = min( minFinal, n->score );
            continue;;
        }
    ...
    for ( int d = 0 ; d < D ; d++ ){
        int idx = mz.ey*X*D + mz.ex*D + d;
        if ( nodes[ idx ].score == minFinal ) {
            walkBack( &mz, &nodes[ idx ] );
        }
    }
    printf( "\n%s\n", mz.bytes );
